...
writer = SummaryWriter("runs/Boston/runs"+datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))
...
for i in range(100):
    yhat = x_train @ w + b
    mse_train = ((yhat-y_train)**2).sum()/x_train.size(0)

    prediction = x_test @ w + b
    mse_test = ((prediction-y_test)**2).sum()/x_test.size(0)
...
def gradient_mini_batch(X_train, Y_train, X_test, Y_test, epsilon, N, writter):
    ...
    for i in range(100):
        ...
        yhat = X_train[draw] @ w + b
        mse_train = ((yhat-Y_train[draw])**2).sum()/N
...
def gradient_SGD(X_train, Y_train, X_test, Y_test, epsilon, writter):
    ...
    for i in range(100):
        ...
        yhat = X_train[draw] @ w + b
        mse_train = ((yhat-Y_train[draw])**2).sum()
